...
class RankingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'level' => 'required|integer|min:1|unique:rankings,level',
            'minimum_earnings' => 'required|regex:/^\d+(\.\d{1,2})?$/|unique:rankings,minimum_earnings',
            'minimum_deposit' => 'required|regex:/^\d+(\.\d{1,2})?$/',
            'minimum_tasks' => 'required|integer|min:0',
            'minimum_task_earning' => 'required|regex:/^\d+(\.\d{1,2})?$/',
        ...
        Ranking::create([
            ...
            'minimum_task_earning' => $input['minimum_task_earning'],
    ...
    public function update(Request $request, Ranking $ranking)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'minimum_deposit' => 'required|regex:/^\d+(\.\d{1,2})?$/',
            'level' => 'required|integer|min:1|unique:rankings,level,' . $ranking->id,
            'minimum_tasks' => 'required|integer|min:0',
            'minimum_task_earning' => 'required|regex:/^\d+(\.\d{1,2})?$/',
            'minimum_referral' => 'required|regex:/^\d+(\.\d{1,2})?$/',
            'minimum_referral_deposit' => 'required|regex:/^\d+(\.\d{1,2})?$/',
